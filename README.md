...
This repository includes a Vercel Serverless Function proxy:

```txt
api/deepseek-plan.js
```

The frontend no longer stores a real DeepSeek API key. The request flow is:

```txt
GitHub Pages / Vercel frontend → /api/deepseek-plan → DeepSeek API
```
...
HRI_PROXY_TOKEN=a random proxy access token
ALLOWED_ORIGINS=https://supermoonzzz.github.io,http://localhost:8000
```

If the whole project is hosted on Vercel, the frontend automatically calls same-origin `/api/deepseek-plan`.

The current GitHub Pages frontend defaults to the HRI proxy endpoint deployed under the existing travel destination agent Vercel project. It reuses that project's DeepSeek configuration and requires a proxy access token:

```txt
https://travel-destination-agent-demo-verce.vercel.app/api/hri-plan
```

To switch to another proxy, enter and save a different Vercel proxy endpoint and its access token on the scene page, for example:
