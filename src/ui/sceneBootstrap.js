...
(function(){
  ...
  function exportKnownFunctions(){
    ...
    const names = ['submitSceneText','demoAmbiguousWater','downloadDefaultMap','saveSceneProxyUrl','saveSceneApiKey','demoStop'];
    names.forEach(name => {
      try {
  ...
  function bindButtons(){
    ...
    cloneAndBind('saveSceneProxyUrlBtn', () => {
      if (hasFn('saveSceneProxyUrl')) return window.saveSceneProxyUrl();
      if (hasFn('saveSceneApiKey')) return window.saveSceneApiKey();
      report('保存代理按钮不可用：saveSceneProxyUrl 没有加载。请检查 src/robot/robotStateMachine.js。');
    });
