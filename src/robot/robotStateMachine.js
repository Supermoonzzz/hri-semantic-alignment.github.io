...
const PROXY_URL_STORAGE_KEY = 'HRI_DEEPSEEK_PROXY_URL';
const PROXY_TOKEN_STORAGE_KEY = 'HRI_DEEPSEEK_PROXY_TOKEN';

function defaultDeepSeekProxyUrl(){
  const configured = window.HRI_PROXY_CONFIG?.deepseekProxyUrl || window.HRI_API_PROXY_URL || '';
  if (configured) return configured;
  const host = window.location.hostname;
  if (host === 'localhost' || host === '127.0.0.1' || host.endsWith('.vercel.app')) return '/api/deepseek-plan';
  return '';
}
...
function readSceneProxyUrl(){
  const fromInput = $('sceneProxyUrl')?.value || '';
  return normalizeProxyUrl(fromInput || localStorage.getItem(PROXY_URL_STORAGE_KEY) || defaultDeepSeekProxyUrl());
}
function readSceneProxyToken(){
  return String($('sceneProxyToken')?.value || localStorage.getItem(PROXY_TOKEN_STORAGE_KEY) || '').trim();
}
function saveSceneProxyUrl(){
  const url = normalizeProxyUrl($('sceneProxyUrl')?.value || '');
  const token = readSceneProxyToken();
  if (!url) {
    localStorage.removeItem(PROXY_URL_STORAGE_KEY);
    localStorage.removeItem(PROXY_TOKEN_STORAGE_KEY);
    if ($('aiStatus')) $('aiStatus').textContent = '已清除后端代理地址，将使用本地规则推理。';
    setBubble('已清除后端代理地址。现在点击“进行意图推理”会使用本地语义地图规则。', { force:true });
    return;
  }
  localStorage.setItem(PROXY_URL_STORAGE_KEY, url);
  if (token) localStorage.setItem(PROXY_TOKEN_STORAGE_KEY, token);
  else localStorage.removeItem(PROXY_TOKEN_STORAGE_KEY);
  if ($('aiStatus')) $('aiStatus').textContent = token ? '后端代理与访问口令已保存，本页推理将优先调用 DeepSeek 代理。' : '后端代理已保存，但尚未填写访问口令。';
  setBubble(token ? '后端代理与访问口令已保存。现在可以输入“我想喝水”，然后点击“进行意图推理”。' : '后端代理地址已保存。若该代理要求访问口令，请先填写口令再推理。', { force:true });
}
function initializeProxyUrlField(){
  const url = readSceneProxyUrl();
  if (url && $('sceneProxyUrl')) $('sceneProxyUrl').value = url;
  const token = readSceneProxyToken();
  if (token && $('sceneProxyToken')) $('sceneProxyToken').value = token;
}
function readSceneApiKey(){ return readSceneProxyUrl(); }
function saveSceneApiKey(){ return saveSceneProxyUrl(); }
function initializeApiKeyField(){ return initializeProxyUrlField(); }
