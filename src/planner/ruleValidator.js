...
(function(){
  ...
  async function callDeepSeekMultiPlanV9(inputText){
    const proxyUrl = readSceneProxyUrl?.();
...
【严格输出 JSON，不要输出 Markdown，不要输出解释文字】
{
  ...
  "tasks": [
    ...
    const proxyToken = readSceneProxyToken?.();
    if (proxyToken) proxyHeaders['X-HRI-Proxy-Token'] = proxyToken;
    const resp = await fetch(proxyUrl, {
      method:'POST', headers:proxyHeaders,
      body:JSON.stringify({ prompt, temperature:0.02 })
    });
